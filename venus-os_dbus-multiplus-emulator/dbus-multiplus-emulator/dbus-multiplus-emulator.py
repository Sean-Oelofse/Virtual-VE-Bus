...
class DbusMultiPlusEmulator:
    ...
    def _dbus_value_changed(
        ...
        if (
            dbusServiceNameAcLoad == ""
            and dbusServiceName.startswith("com.victronenergy.acload")
        ) or (dbusServiceNameAcLoad != "" and dbusServiceName == dbusServiceNameAcLoad):
            self.acloadValues.update({str(dbusPath): changes["Value"]})

        if (
            dbusServiceNamePvInverter == ""
            and dbusServiceName.startswith("com.victronenergy.pvinverter")
        ) or (dbusServiceNamePvInverter != "" and dbusServiceName == dbusServiceNamePvInverter):
            self.pvInverterValues.update({str(dbusPath): changes["Value"]})
    ...
    def _update(self):
        ...
        self._dbusservice["/Ac/Out/L1/V"] = (
            self.acloadValues["/Ac/L1/Voltage"]
            if self.acloadValues["/Ac/L1/Voltage"] is not None and "L1" in phase_used
            else self._dbusservice["/Ac/ActiveIn/L1/V"] if "L1" in phase_used else None
        )
        self._dbusservice["/Ac/Out/L1/I"] = (
            (
                self.acloadValues["/Ac/L1/Current"]
                if self.acloadValues["/Ac/L1/Current"] is not None
                else (
                    round(
                        self._dbusservice["/Ac/Out/L1/P"]
                        / self._dbusservice["/Ac/Out/L1/V"],
                    ...
                    and self._dbusservice["/Ac/Out/L1/P"] is not None
                    else 0
                )
            )
            if "L1" in phase_used
        ...
        self._dbusservice["/Ac/Out/L2/V"] = (
            self.acloadValues["/Ac/L2/Voltage"]
            if self.acloadValues["/Ac/L2/Voltage"] is not None and "L2" in phase_used
            else self._dbusservice["/Ac/ActiveIn/L2/V"] if "L2" in phase_used else None
        )
        self._dbusservice["/Ac/Out/L2/I"] = (
            (
                self.acloadValues["/Ac/L2/Current"]
                if self.acloadValues["/Ac/L2/Current"] is not None
                else (
                    round(
                        self._dbusservice["/Ac/Out/L2/P"]
                        / self._dbusservice["/Ac/Out/L2/V"],
                    ...
                    and self._dbusservice["/Ac/Out/L2/P"] is not None
                    else 0
                )
            )
            if "L2" in phase_used
        ...
        self._dbusservice["/Ac/Out/L3/V"] = (
            self.acloadValues["/Ac/L3/Voltage"]
            if self.acloadValues["/Ac/L3/Voltage"] is not None and "L3" in phase_used
            else self._dbusservice["/Ac/ActiveIn/L3/V"] if "L3" in phase_used else None
        )
        self._dbusservice["/Ac/Out/L3/I"] = (
            (
                self.acloadValues["/Ac/L3/Current"]
                if self.acloadValues["/Ac/L3/Current"] is not None
                else (
                    round(
                        self._dbusservice["/Ac/Out/L3/P"]
                        / self._dbusservice["/Ac/Out/L3/V"],
                    ...
                    and self._dbusservice["/Ac/Out/L3/P"] is not None
                    else 0
                )
            )
            if "L3" in phase_used
